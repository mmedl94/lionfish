...
For 2D plots, brushing can be used for data selection by pressing the left
mouse button and encircling the data to be selected.

For tours, the projection axes can be manipulated via drag and drop with the
right mouse button. (Note for Mac users using trackpads: two fingers
have to be used to right-click)

![](https://raw.githubusercontent.com/mmedl94/lionfish_figures/main/figures/2d_tour_demo_short.gif){width=100%}
